...
class LayerNorm2d(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # The fp32 cast is only needed under autocast. Entering an autocast
        # context unconditionally costs real time when AMP is off -- this layer
        # runs 32 times per forward pass, so the overhead is not incidental.
